...
export const NoteProvider = ({ children }) => {
  ...
  const [sharedNotes, setSharedNotes] = useState([]); // New state for shared notes
  ...
  const fetchSharedNotes = async (email) => {
    if (!isLoaded || !isSignedIn) return;
    console.log('Fetching shared notes for:', email);
    try {
      const response = await fetch(`http://localhost:5000/api/note/shared/${email.toLowerCase()}`);
      if (response.ok) {
        const fetchedSharedNotes = await response.json();
        console.log('Fetched shared notes:', fetchedSharedNotes);
        setSharedNotes(fetchedSharedNotes); // Make sure this line is present
      } else {
  ...
  return (
    <NoteContext.Provider value={{ notes, sharedNotes, setSharedNotes, addNote, updateNote, deleteNote, fetchNotes, shareNote, fetchSharedNotes }}>
      {children}
